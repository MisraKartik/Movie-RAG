...
def _get_secret(key: str) -> str:
    ...
    try:
        ...
        st.error(
            f"Missing `{key}`. Add it to a local `.env` file, or, on Streamlit "
            f"Community Cloud, to your app's **Settings → Secrets**."
        )
        st.stop()
...
#  PAGE CONFIG
# ═══════════════════════════════════════════════════════════════════════════════
# ── Resolve secrets (works with local .env AND Streamlit Cloud secrets) ────
...
Question: {question}"""
    ...
    db = QdrantVectorStore(
        ...
        # must match the payload keys written by upload_to_qdrant.py
